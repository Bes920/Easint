/**
 * AI Chat JavaScript - FIXED VERSION
 * Handles chatbot functionality for EASINT platform
 * 
 * Location: static/js/ai_chat.js
 */
...
/**
 * Handle investigation selection change - FIXED
 */
async function onInvestigationChange(event) {
    ...
    try {
        ...
        const resultsResponse = await fetch(`/api/investigations/${investigationId}/results`);
        const resultsData = await resultsResponse.json();
        
        const resultsCount = resultsData.success ? resultsData.results.length : 0;
        ...
        addMessageToUI('Error loading investigation details. Please try again.', 'bot');
    }
}
